...
time=np.array([0, 60, 120, 180,240, 300, 360, 420, 480])
#Create the first item froeach key
dbase={} 
data=pd.read_excel('/home/ev/Documents/RESEARCH/LastWeekProject/PilotTRim/output'+str(0)+'.xlsx', 'Sheet1') 
data=np.array(data)
...
writer = ExcelWriter('ALLwells.xlsx')
...
def polynome(x,y,n, name):
    ...
    #print(y)
    polyfit(x[2:5], y[2:5], n, name, c=False) 
...
for color, column in zip(colors, range(10)): # 10= the number of columns
    wt0key, rel0key=  strainKeys(column)
    wt0=KeysDat(wt0key)
